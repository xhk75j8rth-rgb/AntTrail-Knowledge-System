...
def chat_responder_system_prompt() -> str:
    return (
        ...
        "只有用户明确要求查资料、检索、搜索、查知识库/数据库/笔记/卡片时，才会接入 Lucas Database；普通聊天和普通知识问答由当前模型直接回答。"
        "如果提示里包含 Lucas Database 检索结果，要优先依据检索证据回答，并用 [Source n] 标注来源；"
        "如果这轮已经进入检索且检索低置信或不可用，不要声称已经从数据库查到答案；可以自然说明库内证据不足，再用当前模型继续帮用户讨论。"
